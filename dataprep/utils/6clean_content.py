...
clean_article_path = 'example_entities/clean_articles/'

for lang in tqdm(languages.split('|')):
    articles = json.load(open(article_path+lang+'.json','r'))
    clean_articles = {title:clean_article(content) for title,content in articles.items()}
    json.dump(clean_articles,open(clean_article_path+lang+'.json','w'))
